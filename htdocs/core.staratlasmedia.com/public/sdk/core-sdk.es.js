const M = globalThis, j = M.ShadowRoot && (M.ShadyCSS === void 0 || M.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, k = /* @__PURE__ */ Symbol(), V = /* @__PURE__ */ new WeakMap();
let et = class {
  constructor(t, e, s) {
...
const ht = (r) => new et(typeof r == "string" ? r : r + "", void 0, k), at = (r, ...t) => {
...
const { is: ct, defineProperty: dt, getOwnPropertyDescriptor: pt, getOwnPropertyNames: ut, getOwnPropertySymbols: $t, getPrototypeOf: ft } = Object, N = globalThis, K = N.trustedTypes, _t = K ? K.emptyScript : "", At = N.reactiveElementPolyfillSupport, S = (r, t) => r, H = { toAttribute(r, t) {
  switch (t) {
    case Boolean:
      r = r ? _t : null;
      break;
  ...
  return e;
} }, z = (r, t) => !ct(r, t), Z = { attribute: !0, type: String, converter: H, reflect: !1, useDefault: !1, hasChanged: z };
...
y.elementStyles = [], y.shadowRootOptions = { mode: "open" }, y[S("elementProperties")] = /* @__PURE__ */ new Map(), y[S("finalized")] = /* @__PURE__ */ new Map(), At?.({ ReactiveElement: y }), (N.reactiveElementVersions ??= []).push("2.1.2");
const L = globalThis, J = (r) => r, T = L.trustedTypes, F = T ? T.createPolicy("lit-html", { createHTML: (r) => r }) : void 0, st = "$lit$", f = `lit$${Math.random().toFixed(9).slice(2)}$`, it = "?" + f, mt = `<${it}>`, m = document, w = () => m.createComment(""), C = (r) => r === null || typeof r != "object" && typeof r != "function", B = Array.isArray, yt = (r) => B(r) || typeof r?.[Symbol.iterator] == "function", D = `[ 	
\f\r]`, E = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, G = /-->/g, Q = />/g, _ = RegExp(`>|${D}(?:([^\\s"'>=/]+)(${D}*=${D}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), X = /'/g, Y = /"/g, rt = /^(?:script|style|textarea|title)$/i, gt = (r) => (t, ...e) => ({ _$litType$: r, strings: t, values: e }), vt = gt(1), g = /* @__PURE__ */ Symbol.for("lit-noChange"), d = /* @__PURE__ */ Symbol.for("lit-nothing"), tt = /* @__PURE__ */ new WeakMap(), A = m.createTreeWalker(m, 129);
function nt(r, t) {
...
const Et = (r, t) => {
  ...
  for (let a = 0; a < e; a++) {
    const h = r[a];
    let c, p, l = -1, u = 0;
    for (; u < h.length && (o.lastIndex = u, p = o.exec(h), p !== null); ) u = o.lastIndex, o === E ? p[1] === "!--" ? o = G : p[1] !== void 0 ? o = Q : p[2] !== void 0 ? (rt.test(p[2]) && (i = RegExp("</" + p[2], "g")), o = _) : p[3] !== void 0 && (o = _) : o === _ ? p[0] === ">" ? (o = i ?? E, l = -1) : p[1] === void 0 ? l = -2 : (l = o.lastIndex - p[2].length, c = p[1], o = p[3] === void 0 ? _ : p[3] === '"' ? Y : X) : o === Y || o === X ? o = _ : o === G || o === Q ? o = E : (o = _, i = void 0);
    const $ = o === _ && r[a + 1].startsWith("/>") ? " " : "";
    n += o === E ? h + mt : l >= 0 ? (s.push(c), h.slice(0, l) + st + h.slice(l) + f + $) : h + f + (l === -2 ? a : $);
  }
    ...
    for (; (i = A.nextNode()) !== null && h.length < a; ) {
      if (i.nodeType === 1) {
        if (i.hasAttributes()) for (const l of i.getAttributeNames()) if (l.endsWith(st)) {
          ...
          h.push({ type: 1, index: n, name: U[2], strings: $, ctor: U[1] === "." ? bt : U[1] === "?" ? wt : U[1] === "@" ? Ct : R }), i.removeAttribute(l);
...
xt?.(P, O), (L.litHtmlVersions ??= []).push("3.3.2");
const Ot = (r, t, e) => {
...
let x = class extends b {
  constructor() {
    super(...arguments), this.apiBase = "https://core.staratlasmedia.com/api/v1", this.status = "idle";
  }
  connectedCallback() {
    super.connectedCallback(), this.status = window.StarAtlasCore?.siteCode ? "configured" : "missing-config";
  }
  render() {
    return vt`<span data-core-status=${this.status}>Core SDK ${this.status}</span>`;
  }
};
x.styles = at`
    :host {
...
x = q([
  Mt("star-atlas-core-status")
], x);
function zt() {
  return window.StarAtlasCore;
}
export {
  x as StarAtlasCoreStatus,
  zt as getCoreConfig
};
